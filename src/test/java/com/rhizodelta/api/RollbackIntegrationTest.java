...
class RollbackIntegrationTest {
    ...
    void rollbackShouldRemoveMergeDecisionAndDisappearFromAuditList() {
        ...
        ResponseEntity<Map> mergeResponse = restTemplate.postForEntity(
                "/api/decisions/merge",
                mergeRequest("rollback-merge-001", sourceNodeId, List.of(synthesizedA)),
                Map.class
        );
        ...
        ResponseEntity<Map> rollbackResponse = restTemplate.exchange(
                "/api/decisions/rollback-merge-001/rollback",
                HttpMethod.POST,
                HttpEntity.EMPTY,
                Map.class
        );
    ...
    void rollbackShouldReturn409WhenDownstreamDependentsExist() {
        ...
        ResponseEntity<Map> aResponse = restTemplate.postForEntity(
                ...
                branchRequest("rollback-branch-a", rootNodeId, "A"),
                Map.class
        );
        ...
        ResponseEntity<Map> bResponse = restTemplate.postForEntity(
                "/api/decisions/branch",
                branchRequest("rollback-branch-b", nodeA, "B"),
                Map.class
        );
        ...
        ResponseEntity<Map> rollbackAResponse = restTemplate.exchange(
                "/api/decisions/rollback-branch-a/rollback",
                HttpMethod.POST,
                HttpEntity.EMPTY,
                Map.class
        );
    ...
    void rollbackShouldAlsoRemoveSemanticAssociations() {
        ...
        ResponseEntity<Map> branchResponse = restTemplate.postForEntity(
                "/api/decisions/branch",
                branchRequest("rollback-semantic-001", sourceNodeId, "semantic branch"),
                Map.class
        );
        ...
        ResponseEntity<Map> associationResponse = restTemplate.postForEntity(
                "/api/associations",
                associationRequest(UUID.fromString(decisionNodeId), relatedNodeId),
                Map.class
        );
        ...
        ResponseEntity<Map> rollbackResponse = restTemplate.exchange(
                "/api/decisions/rollback-semantic-001/rollback",
                HttpMethod.POST,
                HttpEntity.EMPTY,
                Map.class
        );
